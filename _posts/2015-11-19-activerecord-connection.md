...
yet anotherなActiveRecordのShardingライブラリを設計する為、コネクション管理と実際にコネクションがどのように使用されているのかを書き留めておく。
...
1. キューからコネクションを取得
2. 取得出来なかった場合、プールの上限には達していなければ新規接続
3. プール上限に達してた場合、timeout付きで再度キューから取得できるのを待つ。だめなら例外
